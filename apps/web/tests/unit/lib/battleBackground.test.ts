...
describe("getBattleBackgroundImage", () => {
  ...
  it("returns each region's own background when art exists", () => {
    ...
    expect(getBattleBackgroundImage(false, "mountain")).toBe(
      "/backgrounds/background_mountain.png",
    );
    expect(getBattleBackgroundImage(false, "bandit")).toBe("/backgrounds/background_bandit.png");
    expect(getBattleBackgroundImage(false, "ruins")).toBe("/backgrounds/background_ruins.png");
  });
